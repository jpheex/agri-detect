...
- common_name 必須使用格式「具體俗名（X屬植物）」，例如「尤加利葉（桉樹屬植物）」、「杜鵑花（杜鵑花屬植物）」；具體俗名優先採民間常用名稱，不可只寫「XX屬植物」。
...
class PlantDiagnosisResult(BaseModel):
    ...
    common_name: str = Field(description="格式：具體俗名（X屬植物），如尤加利葉（桉樹屬植物）")
    scientific_name: str
...
def format_plant_display_name(common_name: str, scientific_name: str = "") -> str:
    """正規化植物顯示名稱為「具體俗名（X屬植物）」。"""
    name = (common_name or "").strip()
    if not name:
        return "未確定"
    if "（" in name and "）" in name:
        return name
    if name.endswith("屬植物"):
        base = name.replace("屬植物", "").strip()
        if base:
            return f"{base}（{name}）"
    genus = (scientific_name or "").split()[0].strip()
    if genus and genus.lower() not in {"n/a", "unknown", "spp.", "spp"}:
        return f"{name}（{genus} 屬植物）"
    return name
...
def diagnosis_to_app_format(result: PlantDiagnosisResult, source: str = "Gemini") -> dict[str, Any]:
    ...
    crop = format_plant_display_name(result.common_name, result.scientific_name)
    ...
    return {
        ...
        "plant_identity": {
            "common_name": crop,
            "specific_name": result.common_name,
            "scientific_name": result.scientific_name,
        },
